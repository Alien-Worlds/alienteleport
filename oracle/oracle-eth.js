...
const provider = new ethers.providers.StaticJsonRpcProvider(config.eth.endpoint);
...
const chain_id = Number.parseInt(config.eth.chain_id);
...
const process_teleported = async (from_block, to_block) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (res.length) {
                for (let r = 0; r < res.length; r++) {
                    const data = ethers.utils.defaultAbiCoder.decode(['string', 'uint', 'uint'], res[r].data);
