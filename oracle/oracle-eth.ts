...
const network = config.eth.network;
 ...
 */
const load_block_number_from_file = async (blocks_file: string = blocks_file_name) => {
...
const save_block_to_file = async (block_num: number, blocks_file: string = blocks_file_name) => {
...
export const extractEthClaimedData = (data: ethers.utils.Result): eosio_claim_data => {
  const id = data[0].toNumber();
  const to_eth = data[1].replace('0x', '') + '000000000000000000000000';
  const quantity = (data[2].toNumber() / Math.pow(10, precision)).toFixed(precision) + ' ' + config.symbol;
  return { oracle_name: config.eos.oracleAccount, id, to_eth, quantity, };
};

export const extractEthTeleportData = (data: ethers.utils.Result, transactionHash: string): eosio_teleport_data => {
...
const process_claimed = async (from_block: number, to_block: number, submit_to_blockchain: boolean) => {
  const query = {
  ...
  for await (const { transactionHash, data } of res) {
    ...
    try {
      ...
      if (e.message.indexOf('Already marked as claimed') > -1) {
        ...
        console.log(`ID ${eosioData.id} is already claimed, account 0x${eosioData.to_eth.substring(0, 40)
...
const process_teleported = async (from_block: number, to_block: number, submit_to_blockchain: boolean) => {
  const query = {
    fromBlock: from_block,
    toBlock: to_block,
    address: config.eth.teleportContract,
...
const run = async (start_ref: 'latest' | number, submit_to_blockchain: boolean) => {
